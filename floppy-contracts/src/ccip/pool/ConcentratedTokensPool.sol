...
import { IERC165 } from "@chainlink/contracts-ccip/src/v0.8/vendor/openzeppelin-solidity/v5.0.2/contracts/utils/introspection/IERC165.sol";
import { EnumerableSet } from "@chainlink/contracts-ccip/src/v0.8/vendor/openzeppelin-solidity/v5.0.2/contracts/utils/structs/EnumerableSet.sol";
import { ITokensManager } from "../interface/ITokensManager.sol";
/// @notice Base abstract class with common functions for all token pools.
/// A token pool serves as isolated place for holding tokens and token specific logic
/// that may execute as tokens move across the bridge.

abstract contract ConcentratedTokensPool is IPoolV1, OwnerIsCreator {
  ...
  event ChainConfigured(uint64 remoteChainSelector, RateLimiter.Config outboundRateLimiterConfig, RateLimiter.Config inboundRateLimiterConfig);
  ...
  struct ChainUpdate {
    ...
    bytes[] remoteTokenAddresses; // Address of the remote token, ABI encoded in the case of a remote EVM chain.
    address[] tokens; // Address of the token will be mapped to the remote token address.
  ...
  /// @notice Gets the IERC20 tokens that this pool can lock or burn.
  ...
  function _validateReleaseOrMint(
    ...
    if (configuredRemotePool.length == 0 || keccak256(releaseOrMintIn.sourcePoolAddress) != keccak256(configuredRemotePool)) {
  ...
  function applyChainUpdates(
    ...
    for (uint256 i = 0; i < chains.length; ++i) {
      ...
      if (update.allowed) {
        ...
        emit ChainAdded(
          update.remoteChainSelector, update.remoteTokenAddresses, update.tokens, update.outboundRateLimiterConfig, update.inboundRateLimiterConfig
        );
      } else {
  ...
  function setChainRateLimiterConfig(uint64 remoteChainSelector, RateLimiter.Config memory outboundConfig, RateLimiter.Config memory inboundConfig) external {
  ...
  function _setRateLimitConfig(uint64 remoteChainSelector, RateLimiter.Config memory outboundConfig, RateLimiter.Config memory inboundConfig) internal {
    if (!isSupportedChain(remoteChainSelector)) {
      revert NonExistentChain(remoteChainSelector);
    }
  ...
  function _updateRemoteChainConfig(
    ...
    for (uint256 i = 0; i < update.tokens.length; ++i) {
      ...
      s_remoteChainConfigs[update.remoteChainSelector].remoteTokenAddresses[update.tokens[i]] = update.remoteTokenAddresses[i];
    }
